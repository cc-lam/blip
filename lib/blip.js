 ...
 * @access private
 */
encoder.__encode = function(input, fn) {
	try {
		...
		if(input.readable !== undefined && input.readable === false) {
			...
			tmpFile = fs.createWriteStream(blip.__tmpFile(), { encoding: 'utf8' })
			gzip.on('close', function() {
				// create tmp file...
				tmpFile = fs.createReadStream(blip.__tmpFile(), { encoding: 'utf8' })
				var totalBytes = fs.statSync(blip.__tmpFile()).size
...
blip.__tmpFile = function() {
	return os.tmpdir() + '/blip_' + process.pid + '.tmp.gz'
}
